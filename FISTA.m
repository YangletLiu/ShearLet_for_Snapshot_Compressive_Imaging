function X = FISTA(iteration,I,H,H_r,G,M,Fai,y,L,lambda)
    ...
    s = zeros(size(H));
    S = zeros(size(H));
    B = fft2(y); % here we only need to transform matrix y to the Fourier domain to initalize the X, or B
    X1 = B; 
    t1 = 1;
    for k=1:iteration
