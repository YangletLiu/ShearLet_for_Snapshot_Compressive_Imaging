...
%% RUN
    tic
    x_ista	= abl_curve(A, AT, x0, y, LAMBDA, L, sigma, niter, COST, bFig,bPer);
    time = toc;
...
%% DISPLAY
    ...
    save(sprintf("results/curve_kobe_%d.mat",k))
end
function X  = abl_curve(A, AT, x0, b, LAMBDA, L, sigma, iteration, COST, bFig, bPer)
if (nargin < 11)
    bPer = false;
end
...
for i = 1:iteration
    ...
    x = ifft2(X);
    x = projection(x);
    x = curveletShrinkage(x,sigma(i),bPer);
    
end
end

function xRec = curveletShrinkage(Xnoisy,sigma,bPer)
    xRec = zeros(size(Xnoisy));
    coeffs = {};
    for i = 1:8
        % Take curvelet transform
        C = fdct_wrapping(Xnoisy(:,:,i),1,1);
        coeffs{i} = C;
        for s = 2:length(C)
            for w = 1:length(C{s})
                arr = C{s}{w};
    ...
    for i=1:8     
        ...
        xRec(:,:,i) = real(ifdct_wrapping(Ct,1,size(Xnoisy,1),size(Xnoisy,2)));
    end
end
